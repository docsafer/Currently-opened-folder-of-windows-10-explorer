...
CString CWin10ExplorerCurrentFolders::CheckShellSpecialFolder(CString strPath)
{
	...
	if (strPath .Compare(_T("Desktop"))==0 || strPath.Compare(_T("桌面")) == 0)
		fid = FOLDERID_Desktop;
	else if (strPath.Compare(_T("Documents")) == 0 || strPath.Compare(_T("文档")) == 0)
		fid = FOLDERID_Documents;
	else if (strPath.Compare(_T("Music")) == 0 || strPath.Compare(_T("音乐")) == 0)
		fid = FOLDERID_Music;
	else if (strPath.Compare(_T("Pictures")) == 0 || strPath.Compare(_T("图片")) == 0)
		fid = FOLDERID_Pictures;
	else if (strPath.Compare(_T("Videos")) == 0 || strPath.Compare(_T("视频")) == 0)
		fid = FOLDERID_Videos;
	else if (strPath.Compare(_T("Downloads")) == 0 || strPath.Compare(_T("下载")) == 0)
